...
def most_popular_characters(characters=characters, top=5):
    ...
    for char in characters:
        appearances = int(char.appearances) if char.appearances != '' else 0        
        if char.name in cnt and appearances < cnt[char.name]: continue
        cnt.update({char.name : appearances})
    
    return [name[0] for name in cnt.most_common(top)]
...
def max_and_min_years_new_characters(characters=characters):
    ...
    cnt = Counter()
    for char in characters:
        if char.year == '': continue
        cnt.update({char.year : 1})
...
def get_percentage_female_characters(characters=characters):
    ...
    """
    genders = ['Male Characters', 'Female Characters', 'Agender Characters', 'Genderfluid Characters']
    cnt = Counter()
    for char in characters:
        if char.sex in genders:
            cnt.update({char.sex : 1})
    
    return round((cnt['Female Characters'])/(len(list(cnt.elements())))*100, 2)
