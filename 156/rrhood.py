...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    result = {name.lower() if isinstance(name, str)
              else name[0].lower(): [] for name in characters}
    story = text.lower().split("\n")
    for line in story:
        for char in characters:
            found = False
            if isinstance(char, tuple):
                for each in char:
                    if each.lower() in line and not found:
                        result[char[0].lower()].append(story.index(line))
                        found = True
            else:
                if char.lower() in line:
                    result[char.lower()].append(story.index(line))
